...
# To Prevent having to make a string, at the start of each number, 1 wi11 be right, 2 wi11 be left, 3 wi11 be up, and 4 wi11 be down.
...
startPoint = 4999
def moveRight(line, grid, start, distance):
    ...
    if line == 1:
        ...
        for path in range(startPoint,blocksNeeded):
            if grid[path] == None:
                ...
                grid[path] = 3
    return (start + blocksNeeded)
    pass
def moveLeft(line, grid,start, distance):
    ...
    return (start + blocksNeeded)
    pass
def moveUp(line, grid, start, distance):
    ...
    return (start + blocksNeeded)
    pass
def moveDown(line, grid, start, distance):
    blocksNeeded = distance * 1000 + 1
    if line == 1:
        for path in range(startPoint,blocksNeeded):
            if grid[path] == None:
...
def wireOne(pathData):
    ...
    for move in pathData:
        ...
        if move == pathData[0]:
            ...
            if moveString[0] == 1:
                endPoint = moveRight(1, grid, startPoint, fixedMove)
            elif moveString[0] == 2:
                endPoint = moveLeft(1, grid, startPoint, fixedMove)
            elif moveString[0] == 3:
                endPoint = moveUp(1, grid, startPoint, fixedMove)
            elif moveString[0] == 4:
                endPoint = moveDown(1, grid, startPoint, fixedMove)
        else:
            if moveString[0] == 1:
                endPoint = moveRight(1, grid, endPoint, fixedMove)
            elif moveString[0] == 2:
                endPoint = moveLeft(1, grid, endPoint, fixedMove)
            elif moveString[0] == 3:
                endPoint = moveUp(1, grid, endPoint, fixedMove)
            elif moveString[0] == 4:
                endPoint = moveDown(1, grid, endPoint, fixedMove)
    return grid
